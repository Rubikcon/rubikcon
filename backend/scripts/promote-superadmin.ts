...
async function promoteSuperAdmin() {
  const TARGET_EMAIL = "rubikconnexus@gmail.com";
  ...
  try {
    ...
    if (!user) {
      ...
      console.error(
        `Cannot promote a non-existent user. Please ensure the user is registered first.`,
      );
      process.exit(1);
    }
    ...
    console.log(
      `Promoting user '${TARGET_EMAIL}' from '${user.role}' to 'SUPER_ADMIN'...`,
    );
