import { ArrowRight, Instagram, Linkedin, Mail, MessageCircle, Phone, Twitter } from 'lucide-react'
    ...
    <footer className="bg-[#050505] border-t border-white/8">
      ...
      <div className="border-b border-white/8">
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p className="font-display font-extrabold text-white text-lg sm:text-xl">Ready to build your future in tech?</p>
            <p className="text-white/40 text-sm mt-0.5">Join learners across Africa gaining practical, career-ready skills.</p>
          </div>
          <a
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-6 py-12 sm:py-14">
        <div className="grid gap-10 sm:gap-8 sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1.3fr]">
          ...
          {/* Explore */}
          <div>
            <p className="text-xs font-mono uppercase tracking-widest text-white/35 mb-4">Explore</p>
            <ul className="space-y-3">
              {EXPLORE_LINKS.map(link => (
                <li key={link.href}>
                  <a href={link.href} className="text-sm text-white/55 hover:text-[#F5C518] transition-colors inline-block py-0.5">
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          {/* Account */}
          <div>
            <p className="text-xs font-mono uppercase tracking-widest text-white/35 mb-4">Learners</p>
            <ul className="space-y-3">
          ...
          {/* Contact */}
          <div>
            <p className="text-xs font-mono uppercase tracking-widest text-white/35 mb-4">Get in touch</p>
            <ul className="space-y-3 text-sm">
              <li>
                <a href={`mailto:${CONTACT.email}`} className="inline-flex items-center gap-2.5 text-white/55 hover:text-[#F5C518] transition-colors py-0.5 break-all">
                  <Mail size={14} className="shrink-0" /> {CONTACT.email}
                </a>
              </li>
              <li>
                <a href={`tel:${CONTACT.phonePrimary.tel}`} className="inline-flex items-center gap-2.5 text-white/55 hover:text-[#F5C518] transition-colors py-0.5">
                  <Phone size={14} className="shrink-0" /> {CONTACT.phonePrimary.label}
                </a>
              </li>
              <li>
                <a href={`tel:${CONTACT.phoneSecondary.tel}`} className="inline-flex items-center gap-2.5 text-white/55 hover:text-[#F5C518] transition-colors py-0.5">
                  <Phone size={14} className="shrink-0" /> {CONTACT.phoneSecondary.label}
                </a>
              </li>
            </ul>
          </div>
