...
export default function LandingPage() {
  ...
  const activeProfession = FEATURED_PROFESSIONS.find(p => p.id === selectedProfession) ?? FEATURED_PROFESSIONS[0]
  ...
  const homeFacilitators = HOME_FACILITATORS.map(f => {
    const match = apiFacilitators.find(a => a.name.toLowerCase().includes(f.name.toLowerCase()))
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {homeFacilitators.map((facilitator, i) => {
              const initials = facilitator.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
              return (
                <motion.div
